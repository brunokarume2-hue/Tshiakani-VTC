...
# railway variables set TWILIO_ACCOUNT_SID=votre_vraie_valeur 2>&1 | grep -v "already exists" | grep -v "^$" || true
# railway variables set TWILIO_AUTH_TOKEN=votre_vraie_valeur 2>&1 | grep -v "already exists" | grep -v "^$" || true
