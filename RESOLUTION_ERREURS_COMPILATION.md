...
D'après l'analyse du projet, voici les 4 problèmes de compilation :
...
**Cause** : Le fichier Info.plist est automatiquement synchronisé par Xcode (PBXFileSystemSynchronizedRootGroup) et peut être ajouté aux ressources.
...
Un script de nettoyage a été créé pour faciliter la résolution :

```bash
chmod +x corriger-erreurs-compilation.sh
...
3. **Supprimez le DerivedData** (si nécessaire) :
   ```bash
...
**Statut** : Guide de résolution des erreurs de compilation
